 ...
 router.post('/signup', async (req, res) => {
    try {
        ...
        if (JoiMessage.includes('loginId')) {
            if (JoiMessage.includes('at least 3')) { // 아이디가 3글자 미만인 경우
                // JoiMessage : "userId" length must be at least 3 characters long
                validationErrorMessage = '아이디는 3글자 이상이어야 합니다.';
            } else if (JoiMessage.includes('fails to match the required pattern')) { // 정규표현식 규칙에 맞지 않는 경우                
        ...
        } else if (JoiMessage.includes('password')) { // 비밀번호가 4글자 미만인 경우
            ...
            if (JoiMessage.includes('at least 2')) { // 닉네임이 2글자 미만인 경우
